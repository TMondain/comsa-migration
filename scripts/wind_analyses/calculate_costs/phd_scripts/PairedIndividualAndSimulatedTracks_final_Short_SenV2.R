...
setwd("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/WindAnalysis/SimpleSimSprAutFunctionRealComp")
setwd("C:/Users/mondainm/Box Sync/PhD Work/Analyses/GLS analyses/WindAnalysis/SimpleSimSprAutFunctionRealComp")
source("FunctionsSimulateSpringAut_Simple.R")
source("FixedWind_dl_2.r")

setwd("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/WindAnalysis/SimpleSimSprAutFunctionRealComp/Final")
setwd("C:/Users/mondainm/Box Sync/PhD Work/Analyses/GLS analyses/WindAnalysis/SimpleSimSprAutFunctionRealComp/Final")
...
GLS_mig <- read_csv("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/GLS_Summary_Movements_Stopovers/GLS_mig_R.csv")
GLS_mig <- read_csv("C:/Users/mondainm/Box Sync/PhD Work/Analyses/GLS analyses/GLS_Summary_Movements_Stopovers/GLS_mig_R.csv")

# wintering grounds averaged for birds that showed weird mid-winter movements
ms <- read_csv("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/Raw_Tracks_Mig_Schedules/All_Combined/Schedule_AllIndivs_MeanWint.csv")
ms <- read_csv("C:/Users/mondainm/Box Sync/PhD Work/Analyses/GLS analyses/Raw_Tracks_Mig_Schedules/All_Combined/Schedule_AllIndivs_MeanWint.csv")

ms$mig[ms$loc == "Senegal" & ms$lat < 25] <- "Winter"

# positions of all individuals
mp <- read_csv("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/Raw_Tracks_Mig_Schedules/All_Combined/Positions_AllIndivs.csv")
mp <- read_csv("C:/Users/mondainm/Box Sync/PhD Work/Analyses/GLS analyses/Raw_Tracks_Mig_Schedules/All_Combined/Positions_AllIndivs.csv")
...
for(s in 1:length(inds)) {
  ...
  for(m in mig) {
    ...
    if(m == "autumn"){
      ...
      # start loc
      id_s <- subset(GLS_mig, indiv == n_i)[,c(5,4)]
      start <- c(id_s$brd_lon, id_s$brd_lat)
      
      # end loc
      id_e <- subset(GLS_mig, indiv == n_i)[,c(11,10)]
      end <- c(id_e$win_lon, id_e$win_lat)
      
      # set start of senegal tracks to be the first place a bird was tracked at 
      # this will be a shorter track to before
      if(unique(d_m$loc) == "Senegal") {
        start <- as.data.frame(head(subset(mp, indiv == n_i),1)[,c(6,7)])
        start <- c(start$lon, start$lat)
      }
    ...
    if(m == "spring"){
      ...
      # start loc
      id_s <- subset(GLS_mig, indiv == n_i)[,c(11,10)]
      ...
      # end loc
      id_e <- subset(GLS_mig, indiv == n_i)[,c(5,4)]
      end <- c(id_e$brd_lon, id_e$brd_lat)
      
      # set end of senegal tracks to be the last place a bird was tracked at 
      # this will be a shorter track to before
      if(unique(d_m$loc) == "Senegal") {
        end <- as.data.frame(tail(subset(mp, indiv == n_i),1)[,c(6,7)])
        end <- c(end$lon, end$lat)
      }
...
# save(sim_t, file = "simulated_birds_100_finalV2_short_sen")
load("simulated_birds_100_finalV2_short_sen")


ggplot(data = subset(sim_t, r_id == "KT" & indiv == 1), aes(x = lon, y = lat, group = r_id, colour=mig)) +
  geom_path() 
...
# real birds
sim_t <- sim_t %>% group_by(indiv, mig) %>% 
...
# saved at number 18. 
# save(r_sim_out, file = "cost_out_sim_100inds_finalV2_short_sen")
load("cost_out_sim_100inds_finalV2_short_sen")
...
# wintering grounds averaged for birds that showed weird mid-winter movements
ms <- read_csv("C:/Users/tmond/Box Sync/PhD Work/Analyses/GLS analyses/Raw_Tracks_Mig_Schedules/All_Combined/Schedule_AllIndivs_RawWint.csv")
